...
class ControlBot(Bot):
    ...
    def __init__(self, *args, github=None, **kw):
        ...
        self.user_conversion: Dict[int, Conversation] = {}
        self.reload = asyncio.Event()
        self.jobs = []

    async def setup(self):
        self.jobs.append(asyncio.create_task(self.watch_daily_push()))
    ...
    @staticmethod
    def parse_message(message: Message, prefix=" ", length=70):
    ...
    async def watch_daily_push(self):
        logger.info(f"每日推送监控已启动.")
        while True:
            ...
            for ms in self.next_message_settings():
                try:
                    ...
                    for _ in range(delta_sec):
                        ...
                        with db.atomic():
                            messages = (
                                Message.select().where(Message.read == False).join(User).where(User.id == ms.user.id).order_by(Message.time)
                            )
                            mids = []
                    ...
                    logger.debug(f"向用户 {ms.user.uid} 推送时发生错误: {e}")

    @cached_property
    def tree(self):
        ms = lambda **kw: {
            "parse_mode": ParseMode.MARKDOWN,
            "style": MenuStyle(back_text="◀️ 返回", **kw),
        }
        ps = lambda **kw: {
            "parse_mode": ParseMode.MARKDOWN,
        ...
        return transform(
            {
                DMenu("Start", "start", self.on_start, default=True): {
                    DMenu("👤 用户信息", "user_info", self.on_user_info): {
                        ...
                        DMenu("🔑 兑换邀请码", "enter_invite_code", self.on_enter_invite_code),
                    },
                        ...
                        PageMenu(
                            ...
                            **ps(limit=4, limit_items=12),
                            ...
                            Menu(
                                "接受小时",
                                "cmt_hour",
                                self.on_change_message_time,
                                **ms(back_to="message_info"),
                            )
                        },
                        ContentPageMenu(
                            "🕰️ 历史消息",
                    ...
                    DMenu(
                        ...
                        DMenu("🔑 兑换邀请码", "pi_invite", self.on_prime_invite),
                    },
                },
                DMenu("Admin", "admin", self.on_admin): {
                    DMenu("👤 用户管理", "user_admin", self.on_user_admin): {
                            ...
                            PageMenu(
                                ...
                                **ps(limit=5, limit_items=5, back_to="user_admin"),
                                ...
                                PageMenu(
                                    ...
                                    "naic_pcs",
                                    "👇🏼 请选择可用次数:",
                                    [Element(str(h), str(h)) for h in [1, 2, 3, 4, 5, 10, 50]] + [Element("无限", "-1")],
                                    **ps(limit=4, limit_items=5, back_to="user_admin"),
                                    ...
                                    PageMenu(
                                        ...
                                        "naic_slots",
                                        "👇🏼 请选择有效天数:",
                                        [Element(str(h), str(h)) for h in [1, 3, 7, 14, 30, 180]] + [Element("无限", "-1")],
                        ...
                        PageMenu(
                            "🌈 新建特权码",
                            "new_admin_validation_code",
                            "👇🏼 请选择多个特权:",
                            self.items_navc_field,
                            **ps(
                                limit=2,
                                limit_items=6,
                                back_to="user_admin",
                                extras=["__navc_field_confirm"],
                            ),
                        ): {DMenu("选择特权", "navc_field", self.on_invite_code_field_select)},
                        ContentPageMenu(
                            "✉️ 邀请码管理",
                            "invite_code_admin",
                            ...
                            **ps(
                                limit=5,
                                limit_items=10,
                                extras=["__invite_code_all_delete"],
                            ),
                        ): {
                ...
                Menu(
                    ...
                    **ms(back_to="user_admin"),
                    ...
                    Menu(
                        "❗ 确定",
                        "icad_confirm",
                        self.on_invite_code_all_delete,
                        **ms(back_to="user_admin"),
                    )
                },
                ...
                PageMenu(
                    "✅ 确认",
                    "__navc_field_confirm",
                    "👇🏼 请选择天数:",
                    [Element(str(h), str(h)) for h in [30, 90, 180, 360]] + [Element("无限", "-1")],
                    **ps(limit=5, limit_items=5, back_to="user_admin"),
                ): {
                    PageMenu(
                        "接收天数",
                        ...
                        **ps(limit=5, limit_items=5, back_to="user_admin"),
                        ...
                        Menu(
                            "接收数量",
                            "navc_pcs",
                            self.on_new_admin_validation_code,
                            **ms(back_to="user_admin"),
                        )
                    }
                },
    ...
    def set_conversation(self, context: Union[TM, TC], user: User, status: ConversationStatus = None):
        ...
        self.user_conversion[(message.chat.id, user.uid)] = Conversation(context, status) if status else None
    ...
    async def use_code(self, code: str, user: User):
        ...
        if invite_code:
            ...
            if invite_code.role <= user.role and invited:
                return "🚫 邀请码用户组不高于当前用户组."
            if user.role <= UserRole.BANNED:
                return "🚫 被封禁用户无法使用邀请码."
            if invite_code.created_by.id == user.id:
                return "🚫 不能邀请自己."
            with db.atomic() as txn:
                try:
                    Invite.create(by=invite_code.created_by, of=user, to=invite_code.role)
                    user.role = invite_code.role
                    if invite_code.slots > 0:
                        invite_code.slots -= 1
                        if invite_code.slots == 0:
                    ...
                    Invitee = User.alias()
                    invitor = invite_code.created_by
                    ...
                    invitor.save()
                    repo = invite_code.repos.get_or_none()
                    if repo:
            ...
            specs = []
            validation_code = ValidationCode.select().where(ValidationCode.code == code.upper())
            for v in validation_code.iterator():
                if user.role <= UserRole.BANNED:
                    return "🚫 被封禁用户无法使用特权码."
                if v.field == ValidationField.NONE:
                    continue
                existing_v = None
                for uv in user.validations.iterator():
                    if uv.field == v.field and uv.until > datetime.now():
                        existing_v = uv
                        break
                with db.atomic():
                    if not existing_v:
                        until = datetime.now() + timedelta(days=v.days)
                        Validation.create(user=user, field=v.field, until=until)
                        logger.debug(f"用户兑换了特权: {user.uid}")
                    else:
                        existing_v.until += timedelta(days=v.days)
                        existing_v.save()
                        logger.debug(f"用户延期了特权: {user.uid}")
                    specs.append(validation_field_names.get(v.field, "未知功能"))
                    v.field = ValidationField.NONE
                    v.save()
            if specs:
                return f"👌 成功使用该特权码, 您可以使用以下功能了: {', '.join(specs)}."
            else:
                return "🚫 邀请码无效."
    ...
    def useroper(perm: UserRole = UserRole.MEMBER, conversation=False, group=False):
        def deco(func):
            async def wrapper(*args, **kw):
                ...
                if isinstance(context, TM) and context.chat.type not in (ChatType.BOT, ChatType.PRIVATE) and not group:
                    return
                ...
                try:
                    if not conversation:
                        self.set_conversation(context, user, status=None)
    ...
    async def text_handler(self, client: Client, message: TM, user: User):
        ...
        conv = self.user_conversion.get((message.chat.id, user.uid), None)
        if not conv:
            message.continue_propagation()
        if conv.status == ConversationStatus.WAITING_INVITE_CODE:
            ...
            for uid in uids:
                try:
                    uns[uid] = (await client.get_users(uid)).name
                    logger.debug(f"加载用户名: {uid} => {uns[uid]}")
                except BadRequest:
                    pass
            ...
            if isinstance(conv.context, TM):
                conv.context.parameters = {}
            conv.context.parameters["embycode_site"] = message.text
            self.set_conversation(conv.context, user, ConversationStatus.WAITING_EMBYCODE)
            ...
            spec = f"[{user.name}](tg://user?id={user.id}) ([管理](t.me/{client.me.username}?start=__u_{user.id}))"
            Message.create(
    ...
    async def on_start(
        ...
        if isinstance(context, TM):
            ...
            if len(cmds) == 2:
                if cmds[1] == "__sharecode":
                    return await self.to_menu(client, context, "pi_embycode")
                if cmds[1] == "__usecode":
                    return await self.to_menu(client, context, "enter_invite_code")
                elif cmds[1] == "__newcode":
                    return await self.to_menu(client, context, "new_invite_code")
                if cmds[1] == "__prime":
        ...
        return f"🌈 您好 {context.from_user.name}, 欢迎使用 **[Embykeeper](https://github.com/embykeeper/embykeeper)**!"

    @useroper(UserRole.ADMIN)
    async def on_user_validation_empty(self, handler, client: Client, context: TC, parameters: dict, user: User):
        uid = int(parameters["user_id"])
        ur: User = User.get_or_none(uid=uid)
        if not ur:
            return "⚠️ 用户未注册."
        with db.atomic():
            for v in ur.validations.iterator():
                v.delete_instance()
            return f"✅ 用户特权已全部删除."
    ...
    async def on_user_info(
        ...
        fields = []
        for v in user.validations.iterator():
            fields.append(
                (
                    f"{validation_field_names.get(v.field, '未知功能')} (有效期至 {v.until.strftime('%Y-%m-%d')})",
                    v.until,
                )
            )
        fields = sorted(fields, key=lambda x: x[1])
        fields = [s for s, _ in fields]
        if fields:
            msg += "\n\n⏱️ 包月特权:\n\n" + indent("\n".join(fields), " " * 3)
        return msg

    @useroper()
    async def on_new_invite_code(
        self,
        handler,
        client: Client,
        context: Union[TC, TM],
    ...
    @useroper()
    async def on_enter_invite_code(
        self,
        handler,
        client: Client,
        context: Union[TC, TM],
        ...
        self.set_conversation(context, user, ConversationStatus.WAITING_INVITE_CODE)
        return "👉🏼 请输入邀请码:"
    ...
    @useroper()
    async def on_change_message_time(self, handler, client: Client, context: TC, parameters: dict, user: User):
    ...
    async def content_message_list(self, handler, client: Client, context: TC, parameters: dict, user: User):
        ...
        items = [(f"{self.parse_message(m, prefix=f' {i+1}|', length=70)}", str(i + 1), m.id) for i, m in enumerate(ms)]
        if not items:
    ...
    async def on_sys_admin(self, handler, client: Client, context: TC, parameters: dict, user: User):
        ...
        try:
            ...
            if latest_user.name:
                ...
                spec = f"[<已注销>](tg://user?id={latest_user.id})"
    ...
    async def on_user_admin(self, handler, client: Client, context: TC, parameters: dict, user: User):
        self.set_conversation(context, user, ConversationStatus.WAITING_USER)
        return "👇🏼 请选择指令\n\n👉🏼 或输入用户以搜索"

    @useroper(UserRole.ADMIN)
    async def content_list_users(
        self,
        handler,
        client: Client,
        context: Union[TM, TC],
        ...
        if ur:
            ...
            for ir in Invite.select().where(Invite.of == ur).iterator():
                try:
                    ...
                    if i.name:
                        spec = f"[{i.name}](tg://user?id={i.id}) ([管理](t.me/{client.me.username}?start=__u_{i.id}))"
                    else:
                        spec = f"[<已注销>](tg://user?id={i.id})"
    ...
    async def items_naic_role(self, handler, client: Client, context: TC, parameters, user: User):
        return [Element(r.name, str(r.value)) for r in UserRole if UserRole.MEMBER <= r < UserRole.CREATOR and r < user.role]

    @useroper(UserRole.ADMIN)
    async def items_navc_field(self, handler, client: Client, context: TC, parameters, user: User):
        fields = parameters.get("navc_fields", None)
        elements = []
        for r in ValidationField:
            name = r.name
            if fields and r.value in fields:
                name = f"✓ {name}"
            elements.append(Element(name, str(r.value)))
        return elements

    @useroper(UserRole.ADMIN)
    async def on_new_admin_invite_code(self, handler, client: Client, context: TC, parameters: dict, user: User):
    ...
    @useroper(UserRole.ADMIN)
    async def on_new_admin_validation_code(self, handler, client: Client, context: TC, parameters: dict, user: User):
        fields = [ValidationField(int(f)) for f in parameters["navc_fields"]]
        if not fields:
            return f"⚠️ 未选择特权."
        days = int(parameters["navc_days_id"])
        pcs = int(parameters["navc_pcs_id"])
        if days < 0:
            days = 10000
        codes = []
        with db.atomic():
            for _ in range(pcs):
                ...
                with db.atomic():
                    for f in fields:
                        ValidationCode.create(code=code, field=f, days=days, created_by=user)
                codes.append(f"`{code}`")
        logger.info(f"{user.uid} 已生成特权码 {pcs} 个.")
        codes = "\n".join(codes)
        return f"🔑 已创建特权码:\n{codes}"

    @useroper(UserRole.ADMIN)
    async def on_invite_code_field_select(self, handler, client: Client, context: TC, parameters: dict, user: User):
        fields = [ValidationField(int(f)) for f in parameters.get("navc_fields", [])]
        field = ValidationField(int(parameters["navc_field_id"]))
        if field in fields:
            parameters["navc_fields"].remove(field)
        elif fields:
            parameters["navc_fields"].append(field)
        else:
            parameters["navc_fields"] = [field]
        await self.to_menu(client, context, "new_admin_validation_code")
    ...
    @useroper(UserRole.ADMIN)
    async def on_invite_code_details(self, handler, client: Client, context: TC, parameters: dict, user: User):
        ...
        try:
            ...
            if created_by.name:
                ...
                spec = f"[<已注销>](tg://user?id={created_by.id})"
    ...
    async def on_prime_info(
        ...
        if user.role < UserRole.SUPER:
            lines += [
                f"  1. 通过爱发电赞助 (5元给开发者买个小包子).",
                f"  2. [分享](t.me/{client.me.username}?start=__usecode) 1-2 个邀请制 Emby 站点的邀请码.",
                f"  3. 在 [Github](https://github.com/embykeeper/embykeeper) 提交Bug修复或新功能.",
                f"  4. 分享邀请链接 ([生成](t.me/{client.me.username}?start=__newcode)) 给三位用户.",
            ]
        else:
    ...
    async def on_share_embycode_site(
        ...
        self.set_conversation(context, user, ConversationStatus.WAITING_EMBYCODE_SITE)
    ...
    @useroper(UserRole.SUPERADMIN)
    async def content_list_embycodes(self, handler, client: Client, context: TC, parameters: dict, user: User):
        ...
        while True:
            ...
            items = [(f"{i+1}. `{c.code}`\n({c.site})\n", str(i + 1), str(c.id)) for i, c in enumerate(ecs.iterator())]
            if not items:
                if not show_all:
                    if not show:
                        show = True
    ...
    @useroper()
    async def on_request_github_prime(self, handler, client: Client, context: TC, parameters: dict, user: User):
        self.set_conversation(context, user, ConversationStatus.WAITING_GITHUB_USERNAME)
        return "✉️ 如果您是本项目的贡献者, 请输入您的用户名:"

    @useroper(UserRole.ADMIN)
    async def on_user_message(self, handler, client: Client, context: TC, parameters: dict, user: User):
        self.set_conversation(context, user, ConversationStatus.WAITING_MESSAGE)
        return "✉️ 请输入信息:"

    @useroper(UserRole.CREATOR)
    async def on_new_invite_repo(self, handler, client: Client, context: TC, parameters: dict, user: User):
        self.set_conversation(context, user, ConversationStatus.WAITING_GITHUB_USERNAME_CREATE)
    ...
    async def items_message_time(self, handler, client: Client, context: TC, parameters: dict, user: User):
        self.set_conversation(context, user, ConversationStatus.WAITING_MESSAGE_TIME)
